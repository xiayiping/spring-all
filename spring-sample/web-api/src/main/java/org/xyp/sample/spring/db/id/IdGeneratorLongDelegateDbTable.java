...
@Slf4j
public class IdGeneratorLongDelegateDbTable {
    ...
    private final ConcurrentHashMap<Class<?>, BatchIdResult> idHolder = new ConcurrentHashMap<>();
    ...
    private final IdGenDialect defaultDialect = new IdGenDialect();

    private final HashMap<String, IdGenDialect> dialectMap = new HashMap<>();
    ...
    public <T> List<Long> nextId(Class<T> entityClass, int fetchSize, String dialect, JdbcConnectionAccessorFactory factory) {

        val entityName = entityClass.getName();
        val cachedState = idHolder.computeIfAbsent(entityClass, clz -> fetchOrCreateIdBatchStateFromDB(entityClass.getName(), dialect, factory));
        ...
        val newState = idHolder.computeIfPresent(entityClass, (clz, state) -> {
    ...
    private BatchIdResult fetchIdBatchFromDB(String entityName, String dialect, Connection connection) {
        ...
        return WithCloseable.open(() -> connection.prepareStatement(getLastIdSql))
            .map(Fun.thenSelf(ps -> log.info("fetch next batch of entity {} {}", entityName, getLastIdSql)))
    ...
    private void initIdValueToTable(String entityName, BatchIdResult initBatch, String dialect, Connection connection) {
        ...
        WithCloseable.open(() -> connection.prepareStatement(initIdValueSql))
            .map(Fun.thenSelf(ps -> log.info("init id batch info for entity {} {}", entityName, initIdValueSql)))
            .map(Fun.thenSelf(ps -> {
            ...
            .consume(this::initIdValueToTable)
            .closeAndGet()
            .specError(IdGenerationException.class, IdGenerationException::new)
            .get()
        ;
    ...
    private void initIdValueToTable(PreparedStatement preparedStatement) {
        ResultOrError.on(preparedStatement::executeUpdate)
            .map(Fun.thenSelf(i -> log.info("init batch id of entity, inserted row count {}", i)))
            .specError(IdGenerationException.class, IdGenerationException::new)
            .get()
        ;
    }
    ...
    private void updateIdBatch(String entityName, long last, String dialect, Connection connection) {
        ...
        WithCloseable.open(() -> connection.prepareStatement(updateIdSql))
            .map(Fun.thenSelf(ps -> log.info("update batch id for entity {} {} ", entityName, updateIdSql)))
            .map(Fun.thenSelf(ps -> {
                ps.setLong(1, last);
    ...
    private void updateIdBatch(PreparedStatement preparedStatement) {
        ResultOrError.on(preparedStatement::executeUpdate)
            .map(Fun.thenSelf(i -> log.info("update batch id, inserted row count {}", i)))
            .specError(IdGenerationException.class, IdGenerationException::new)
            .get()
        ;
    }
