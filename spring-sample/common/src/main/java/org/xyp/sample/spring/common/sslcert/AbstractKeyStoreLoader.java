...
public abstract class AbstractKeyStoreLoader<T> implements KeyStoreLoader {
    ...
    private SecretStoreBundle getBundle(String bundleName) {
        ...
        Assert.state(null != b, () -> "can't find bundle of name " + bundleName);
        return b;
    }
    ...
    public void loadData(KeyStoreSecretProperty property) {
        ...
        val clientAndPaths = vaultEntries.stream()
            .map(entry -> ResultOrError.on(() -> new SecretTuple<>(
                getReader(entry), entry.getPaths()
            )).get())
            .toList();
        ...
        val bundled = clientAndPaths.stream()
            ...
            .map(tu -> ResultOrError.on(() -> readBundle(
                tu.v1(),
                tu.v2().getPath(),
    ...
    protected Map<String, SecretStoreBundle> readBundle(
        ...
        return bundles.entrySet().stream()
            .map(e -> new SecretTuple<>(
                e.getKey(),
                createStoreBundleFromMap(dataMap, e.getValue())
            ))
            .collect(Collectors.toMap(SecretTuple::v1, SecretTuple::v2));
    }

    protected SecretStoreBundle createStoreBundleFromMap(
        Map<String, ?> map,
        KeyStoreSecretProperty.Bundle bundle
    ) {
        return new SecretStoreBundle(
            Optional.ofNullable(map.get(bundle.getKeyStoreField())).map(s -> Base64.getDecoder().decode(s.toString())).orElse(null),
            Optional.ofNullable(map.get(bundle.getTrustStoreField())).map(s -> Base64.getDecoder().decode(s.toString())).orElse(null),
            Optional.ofNullable(map.get(bundle.getKeyStorePasswordField())).map(s -> Base64.getDecoder().decode(s.toString())).orElse(null),
            Optional.ofNullable(map.get(bundle.getTrustStorePasswordField())).map(s -> Base64.getDecoder().decode(s.toString())).orElse(null)
        );
    }
}
