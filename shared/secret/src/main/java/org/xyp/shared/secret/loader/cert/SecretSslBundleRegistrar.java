...
public class SecretSslBundleRegistrar implements SslBundleRegistrar {
    ...
    public void registerBundles(SslBundleRegistry registry) {
        ...
        if (registry instanceof DefaultSslBundleRegistry bundles) {
            for (val entry : property.getEntries()) {
                val referenced = entry.getFetchSource().equals(KeyStoreFetchSource.REF);
                if (referenced) {
    ...
    private void bundleReferredStore(String newBundleId, KeyStoreSecretProperty.Bundle bundleDetail,
                                     DefaultSslBundleRegistry defaultReg) {
        try {
            String newPassword = generateRandomPassword();
            byte[] keyStoreBytes = null;
            if (StringUtils.hasText(bundleDetail.getKeyStoreField())) {
            ...
            if (StringUtils.hasText(bundleDetail.getTrustStoreField())) {
    ...
    private void mergeKeyStore(KeyStore mergedKeystore, String keyRef,
    ...
    private void mergeTrustStore(KeyStore mergedKeystore, String keyRef,
                                 DefaultSslBundleRegistry defaultReg)
